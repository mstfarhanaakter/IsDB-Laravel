...
<div class="container mx-auto px-4 py-6">
    ...
    <form action="{{ route('qc.inspections.update', $inspection->id) }}" method="POST" class="bg-white p-6 rounded shadow-md">
        @csrf
        @method('PUT')
        ...
        <div class="mb-4">
            ...
            <select name="production_id" class="w-full border px-3 py-2 rounded">
                ...
                @foreach($productions as $production)
                    <option value="{{ $production->id }}" {{ $inspection->production_id == $production->id ? 'selected' : '' }}>{{ $production->name }}</option>
                @endforeach
            </select>
        </div>
        ...
        <div class="mb-4">
            ...
            <input type="text" name="inspector_name" value="{{ $inspection->inspector_name }}" class="w-full border px-3 py-2 rounded">
        </div>

        <div class="mb-4">
            <label class="block mb-1">Inspection Date</label>
            <input type="date" name="inspection_date" value="{{ $inspection->inspection_date->format('Y-m-d') }}" class="w-full border px-3 py-2 rounded">
        </div>
        ...
        <div class="mb-4">
            ...
            <textarea name="remarks" class="w-full border px-3 py-2 rounded">{{ $inspection->remarks }}</textarea>
        </div>
